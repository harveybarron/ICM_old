...
# shape (Ny,Nx)
#l, cl_tt, cl_ee, cl_bb, cl_te = np.loadtxt('/home/reinyv/software/NaMaster/test/cls.txt', unpack=True) # This one doesn't go to high enough l: max(l)=767 which corresponds to 14 arcmin
l, cl_tt, cl_ee, cl_bb, cl_te = np.loadtxt('cls_cmb.txt', unpack=True) # this one goes plenty high enough: max(l) = 9134 corresponding to 1.2 arcmin, around the pixel scale
ell_max=np.pi/(pix*np.pi/180) # max expected l based on pixel size

# Generate random realisation of the power spectrum, just in temperature (spin-0).  Set random seed for repeatability - doesn't work?!
rseed=10
...
cl_tt_conv = cl_tt*beam # beam in example 7
...
sq_sum = np.sum(np.multiply(mpt_conv3,mpt_conv3))*pix**2


# Write out for reuse later
np.save('mpt.npy', mpt)
np.save('mpt_conv.npy', mpt_conv)
np.save('mpt_conv2.npy', mpt_conv2)
np.save('mpt_conv3.npy', mpt_conv3)
...
sum2 = 0.
for i in range(len(cl00_uncoupled_conv3[0])):
    sum2 = sum2 + (cl00_uncoupled_conv3[0])[i]**2*(lf_bins[i] - l0_bins[i])
    
sum3 = si.simpson(cl_tt**2, l)

print("\n The sum of sqaures of map and integrals of power spectrum resp. \n")
print(sq_sum)
print(1/2/np.pi*sum2)
print(1/2/np.pi*sum3)
...
plt.legend(loc='best')
plt.loglog()
plt.savefig("beam_test2.png",dpi=400)
plt.show()
