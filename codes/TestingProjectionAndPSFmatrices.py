...
plt.plot(rs,X,'r.',label='$y_{2D}=R_{PSF}.R_{PROJ}.y_{3D}$ (i.e convoluted)')
plt.plot(rs,Y,'b.',label='$y_{2D}=R_{PROJ}.y_{3D}$ (i.e no convolution)')
plt.plot(rs[:],y_proj[:],'g',label='with LOS integration (no convolution)')
...
plt.plot(rs,S,'r.',label='$y_{3D}=R_{PROJ}^{-1}.R_{PSF}^{-1}.y_{2D}$ (convoluted)')
plt.plot(rs,T,'b.',label='$y_{3D}=R_{PROJ}^{-1}.y_{2D}$ (no convolution)')
plt.plot(rs[:],y_3D[:],'g',label='Original 3D y model')
plt.legend(loc=3, prop={'size': 9})
